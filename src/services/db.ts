...
import { InventoryItem, ImportLog, ProcessingResult } from '../interfaces';
 ...
 */
async function upsertInventoryItems(items: InventoryItem[], importLogId: number): Promise<ProcessingResult> {
  const client = await pool.connect();
 ...
 */
async function createImportLog(filename: string): Promise<number> {
  const client = await pool.connect();
  try {
