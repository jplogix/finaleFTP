...
exports.upsertInventoryItems = upsertInventoryItems;
...
async function initDatabase() {
    ...
    `);
        ...
        logger_1.default.error('Error initializing database', err);
        throw err;
    }
    finally {
        client.release();
 ...
 * @param items - Array of inventory items to insert/update
 * @param importLogId - ID of the import log entry
...
async function upsertInventoryItems(items, importLogId) {
    ...
    try {
        ...
        for (const item of items) {
            try {
                ...
                const checkResult = await client.query('SELECT id FROM inventory_items WHERE item_id = $1', [item.item_id]);
          ...
          `, [
                ...
                logger_1.default.error(`Error processing item ${item.item_id}`, err);
                stats.errors++;
            }
        }
        // Update import log
    ...
    `, [
        ...
        logger_1.default.error('Error in upsertInventoryItems transaction', err);
        ...
        UPDATE import_logs SET
          ...
          completed_at = CURRENT_TIMESTAMP
        WHERE id = $2
      `, [err instanceof Error ? err.message : 'Unknown error', importLogId]);
        }
        catch (logErr) {
            logger_1.default.error('Error updating import log with failure', logErr);
        }
        throw err;
    }
    finally {
...
async function createImportLog(filename) {
    ...
    `, [filename]);
        ...
        logger_1.default.error('Error creating import log', err);
        throw err;
    }
    finally {
        client.release();
